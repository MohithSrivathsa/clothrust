...
pub async fn callback(query: web::Query<CallbackQuery>, session: Session) -> HttpResponse {
    ...
    let token_body = match client.post(&token_url).form(&params).send().await {
        ...
        Err(e) => return HttpResponse::Ok().content_type("text/html").body(
            format!("<h2>Network Error</h2><pre>{}</pre><a href='/'>Back</a>", e)
        ),
    };
    log::info!("Token response: {}", &token_body[..token_body.len().min(200)]);

    let token: TokenResponse = match serde_json::from_str(&token_body) {
        Ok(t)  => t,
        Err(e) => return HttpResponse::Ok().content_type("text/html").body(
            format!("<h2>Token Error</h2><pre>{}</pre><p>{}</p><a href='/'>Back</a>", token_body, e)
        ),
    };
    ...
    let user_body = match client.get(&userinfo_url).bearer_auth(&token.access_token).send().await {
        Ok(r)  => r.text().await.unwrap_or_default(),
        Err(e) => return HttpResponse::Ok().content_type("text/html").body(
            format!("<h2>Userinfo Error</h2><pre>{}</pre><a href='/'>Back</a>", e)
        ),
    };
    log::info!("Userinfo: {}", &user_body[..user_body.len().min(200)]);

    let user: AuthUser = match serde_json::from_str(&user_body) {
        Ok(u)  => u,
        Err(e) => return HttpResponse::Ok().content_type("text/html").body(
            format!("<h2>Profile Error</h2><pre>{}</pre><p>{}</p><a href='/'>Back</a>", user_body, e)
        ),
    };
    ...
    // HTML redirect — gives browser time to save session cookie
...
<div class="s"></div><p style="color:#6b6560">Signing in as {}…</p>
<script>setTimeout(()=>location.href='/profile',200)</script>
</body></html>"#, name
    ))
}
        ...
        Some(u) => {
            let orders: Vec<serde_json::Value> = {
                ...
                let x = match db.prepare(
                    ...
                    Ok(mut stmt) => stmt
                        .query_map([email.as_str()], |row| Ok(serde_json::json!({
            ...
            let html = match data.tera.render("profile.html", &ctx) {
                Ok(h) => h,
                Err(e) => {
                    log::error!("Profile template error: {:?}", e);
                    format!("<h2>Template Error</h2><pre style='font-family:monospace;white-space:pre-wrap'>{}</pre><a href='/'>Back</a>", e)
                }
            };
